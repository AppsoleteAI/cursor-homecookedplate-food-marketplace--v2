...
BEGIN
  ...
  INTO current_maker_count, current_taker_count
  FROM public.metro_area_counts
  WHERE metro_name = area
  FOR UPDATE;
...
COMMENT ON FUNCTION public.increment_metro_count IS 'Atomically increments maker_count or taker_count for a metro area (thread-safe)';
